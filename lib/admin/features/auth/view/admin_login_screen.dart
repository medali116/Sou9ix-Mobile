...
/// The Administrator app's only login screen — real Firebase Auth
/// email/password sign-in (see [AdminAuthService]). Resolves which shop
/// this account owns via the top-level `admins/{uid}` lookup and caches it
/// locally ([ShopCodeStorage]), the same way the Caissier app's join-code
/// flow does — this is what lets an admin log in on a brand-new device and
/// still land on their existing shop.
///
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      setState(() {
        _loading = false;
        _error =
            'Impossible de vous connecter — vérifiez votre connexion internet et réessayez.';
      });
      return;
    }

    await ShopCodeStorage.save(result.shopCode);
    if (!mounted) return;
    ref.read(shopCodeProvider.notifier).state = result.shopCode;
          ...
          children: [
            ...
            SizedBox(
              ...
              child: OutlinedButton(
                onPressed: () => context.push('/login/create'),

...
class _ProfileScreenState extends ConsumerState<ProfileScreen> {
  ...
  Future<void> _openReportProblemSheet(BuildContext context, AppUser? user) async {
    ...
    final logs =
        'Sou9ix POS v1.0.0\n'
        'Utilisateur : ${user?.email ?? 'inconnu'} (Admin)\n'
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 24),
            ...
            const SizedBox(height: 12),
            _ShopCodeCard(
              code: companySettings.companyCode,
              onCopied: () => _toast(context, 'Code copié !'),
            ),
            const SizedBox(height: 12),
            _InfoCard(
              rows: [
...
/// The shop's permanent join code — an employee types this once, on their
/// first Caissier-app launch, to link that install to this shop. Always
/// visible here so the admin can hand it out at any time, not just at
/// signup.
class _ShopCodeCard extends StatelessWidget {
  final String? code;
  final VoidCallback onCopied;
  const _ShopCodeCard({required this.code, required this.onCopied});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.qr_code_rounded, color: Colors.white, size: 26),
          const SizedBox(width: 14),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,

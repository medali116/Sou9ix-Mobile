...
import 'package:sou9ix/shared/features/products/service/categories_repository.dart';
...
final productsRepositoryProvider = Provider<ProductsRepository?>((ref) {
  ...
  return ProductsRepository(FirebaseFirestore.instance, shopCode);
});

/// Every mutator here updates `state` immediately (optimistic, synchronous
/// — exactly as before this file went Firestore-backed) *and* persists the
/// change in the background. This isn't just an optimization: several
/// callers (`SaleService.checkout`, `StockService.recordReturn`,
/// `PurchaseService.receiveInvoice`) read `ref.read(productsProvider)`
/// synchronously in the same tick right after calling a mutator here (e.g.
/// to compute `stockApres` for a stock-movement log entry) — a
/// write-then-wait-for-the-Firestore-echo pattern would silently break
/// every one of those. The Firestore snapshot listener still applies on
/// top of this for changes made by the *other* app.
class ProductsNotifier extends StateNotifier<List<Product>> {
  ProductsNotifier(this._ref, this._repo) : super(_buildMockProducts()) {
    unawaited(_init());
  }
  ...
  Future<void> decrementStock(String productId, double quantite) async {
    decrementStockLocal(productId, quantite);
    await _repo?.incrementStock(productId, -quantite);
  }

  /// Same optimistic local mutation as [decrementStock], without the
  /// persist call — used by [SaleService.checkout], which persists every
  /// line's stock change as part of one combined [SalesRepository.
  /// recordSaleBatch] write instead of one write per product.
  void decrementStockLocal(String productId, double quantite) {
    state = [
  ...
  Future<void> restock(
    ...
    final repo = _repo;
    if (repo == null) return;
    await repo.incrementStock(productId, quantiteRecue);
    if (nouveauPrixAchat != null) {
      await repo.setPurchasePrice(productId, nouveauPrixAchat);
    }
  }
  ...
  Future<void> adjustStock(String productId, double delta) async {
    ...
    await _repo?.incrementStock(productId, delta);
  }
...
final productsProvider = StateNotifierProvider<ProductsNotifier, List<Product>>(
  (ref) => ProductsNotifier(ref, ref.watch(productsRepositoryProvider)),
);

final categoriesRepositoryProvider = Provider<CategoriesRepository?>((ref) {
  final shopCode = ref.watch(shopCodeProvider);
  if (shopCode == null) return null;
  return CategoriesRepository(FirebaseFirestore.instance, shopCode);
});
...
class CategoriesNotifier extends StateNotifier<List<ProductCategory>> {
  ...
  StreamSubscription<List<ProductCategory>>? _sub;

  Future<void> _init() async {
    final repo = _repo;
    if (repo == null) return;
    await repo.bootstrapIfEmpty(mockCategories);
    _sub = repo.watchAll().listen((list) => state = list);
  }
...
final categoriesProvider =
    StateNotifierProvider<CategoriesNotifier, List<ProductCategory>>(
      (ref) => CategoriesNotifier(ref.watch(categoriesRepositoryProvider)),
    );

...
enum _SortOption { defaut, prixAsc, prixDesc, stockAsc, stockDesc, nomAsc, nomDesc, faible }
...
class _ProductsScreenState extends ConsumerState<ProductsScreen> {
          ...
          children: [
            ...
            Padding(
              ...
              child: Align(
                ...
                child: Text('Trier par', style: Theme.of(context).textTheme.titleLarge),
              ),
            ),
            Flexible(
                ...
                child: Column(
                  ...
                  children: [
                    for (final option in _SortOption.values)
                      ListTile(
                        leading: Icon(option.icon, color: _sort == option ? AppColors.teal : AppColors.textFaint),
                        title: Text(
                          option.label,
                          style: TextStyle(
                            fontWeight: _sort == option ? FontWeight.w800 : FontWeight.w500,
                            color: _sort == option ? AppColors.teal : AppColors.textPrimary,
                          ),
                        ),
                        trailing: _sort == option ? const Icon(Icons.check_rounded, color: AppColors.teal) : null,
  ...
  Future<void> _confirmAndDelete(Product p) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('« ${p.name} » sera retiré du catalogue. Cette action est irréversible.'),
        actions: [
  ...
  void _showDetailsSheet(Product p) {
    ...
    final cat = categories.firstWhere((c) => c.id == p.categorieId, orElse: () => categories.first);
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    ProductAvatar(emoji: p.emoji, photoBytes: p.photoBytes, size: 52),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              Icon(cat.icon, size: 13, color: AppColors.textFaint),
                ...
                _detailRow(
                  'Prix de vente',
                  p.venduAuPoids ? '${AppFormat.dtShort(p.prixVente)} / kg' : AppFormat.dtShort(p.prixVente),
                ),
                _detailRow(
                  'Prix d\'achat',
                  p.venduAuPoids ? '${AppFormat.dtShort(p.prixAchat)} / kg' : AppFormat.dtShort(p.prixAchat),
                ),
                _detailRow('Marge', '${p.margePct.toStringAsFixed(0)}%'),
                _detailRow('Stock', p.venduAuPoids ? AppFormat.kg(p.stock) : '${p.stock.toInt()} pcs'),
                _detailRow('Seuil d\'alerte', p.venduAuPoids ? AppFormat.kg(p.seuilAlerte) : '${p.seuilAlerte.toInt()} pcs'),
                if (p.codeBarres != null) _detailRow('Code-barres', p.codeBarres!),
                if (p.datePeremption != null)
                  _detailRow('Péremption', DateFormat('dd/MM/yyyy').format(p.datePeremption!)),
              ],
            ),
          ),
        ),
          ...
          child: Column(
            children: [
              ...
              Padding(
                ...
                child: Align(
                  alignment: Alignment.centerLeft,
                  child: Text('Historique · ${p.name}', style: Theme.of(context).textTheme.titleLarge),
                ),
              ),
              Expanded(
                child: moves.isEmpty
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              Text(DateFormat('dd/MM/yyyy HH:mm').format(m.date),
                ...
                PressScale(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color: _sort == _SortOption.defaut ? AppColors.border : AppColors.teal,
                        width: 1.3,
                      ),
                    ),
                    child: Row(
                      ...
                      children: [
                        Icon(
                          ...
                          size: 19,
                          color: _sort == _SortOption.defaut ? AppColors.textPrimary : AppColors.teal,
                        ),
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final tile = isAdmin
                          ? Dismissible(
                              ...
                              secondaryBackground: _swipeBackground(isEdit: false),
                      ...
                      return tile.animate().fadeIn(duration: 220.ms, delay: (18 * index).ms);
                    },
                  ),
          ),
        ],
...
class _StockTag extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final color = rupture ? AppColors.danger : (faible ? AppColors.warning : AppColors.success);
    final label = rupture
        ? 'Rupture'
